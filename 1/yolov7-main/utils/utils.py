...
def distancing(people_coords, car_coords,img, intermediate, high, normal,dist_thres_lim=(2,3)):
    points_3d = gol.get_value('points_3d')
    #global points_3d #11111111
    already_red = dict() # dictionary to store if a plotted rectangle has already been labelled as high risk
    centers_people = []
    centers_car = []
    centers = []
    for i in people_coords:
        centers_people.append(((int(i[2])+int(i[0]))//2,(int(i[3])+int(i[1]))//2))
    for i_car in car_coords:
        centers_car.append(((int(i_car[2])+int(i_car[0]))//2,(int(i_car[3])+int(i_car[1]))//2))
    centers = centers_people +centers_car
    for j in centers:
        already_red[j] = 2
    #x_combs = list(itertools.combinations(people_coords,2))
    x_combs = itertools.product(people_coords, car_coords, repeat=1)
    radius = 10
    thickness = 5
    for x in x_combs:
        points_3d = gol.get_value('points_3d')
        ...
        '''cetr1 = ((int(xyxy1[1]) + int(xyxy1[3])) // 2, (int(xyxy1[0]) + int(xyxy1[2])) // 2)
        cetr2 = ((int(xyxy2[1]) + int(xyxy2[3])) // 2, (int(xyxy2[0]) + int(xyxy2[2])) // 2)
        tup_0 = (0,)
        tup_1 = (1,)
        tup_2 = (2,)
        dis_1=cetr1 + tup_0
        ...
        a = points_3d[dis_1]
        print(a)'''
        cntr1_0 = (points_3d[(int(xyxy1[1])+int(xyxy1[3]))//2,(int(xyxy1[0])+int(xyxy1[2]))//2, 0],points_3d[(int(xyxy1[1])+int(xyxy1[3]))//2,(int(xyxy1[0])+int(xyxy1[2]))//2, 1],points_3d[(int(xyxy1[1])+int(xyxy1[3]))//2,(int(xyxy1[0])+int(xyxy1[2]))//2, 2]) #1111111
        cntr2_0 = (points_3d[(int(xyxy2[1])+int(xyxy2[3]))//2,(int(xyxy2[0])+int(xyxy2[2]))//2, 0], points_3d[(int(xyxy2[1])+int(xyxy2[3]))//2,(int(xyxy2[0])+int(xyxy2[2]))//2, 1], points_3d[(int(xyxy2[1])+int(xyxy2[3]))//2,(int(xyxy2[0])+int(xyxy2[2]))//2, 2])  # 1111111

        #dist = ((cntr2[0]-cntr1[0])**2 + (cntr2[1]-cntr1[1])**2)**0.5
        dist = ((cntr2_0[0] - cntr1_0[0]) ** 2 + (cntr2_0[1] - cntr1_0[1]) ** 2 + (cntr2_0[2] - cntr1_0[2]) ** 2) ** 0.5
        dist = dist/1000
        ...
        if dist > dist_thres_lim[0] and dist < dist_thres_lim[1]:
            ...
            # cm = round(dist)/100
            cm = round(dist,2)
            cv2.putText(img, str(cm)+" m", mid, cv2.FONT_HERSHEY_SIMPLEX , 1, color, 2, cv2.LINE_AA)
            already_red[cntr1] = 0
            already_red[cntr2] = 0
